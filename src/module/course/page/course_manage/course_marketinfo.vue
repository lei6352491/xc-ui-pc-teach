  ...
  export default {
        ...
        this.$refs.courseMarketForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              this.courseMarketForm.courseid = this.courseid;
    ...
    mounted(){
      ...
      systemApi.sys_getDictionary('203').then((res) => {
        this.chargeList = res.queryOneResult.data.dvalue;
      });
      systemApi.sys_getDictionary('204').then((res) => {
        this.validList = res.queryOneResult.data.dvalue;
      });

      //获取课程营销信息
      courseApi.getCourseMarketById(this.courseid).then((res) => {
        if(res.success){
           this.courseMarketForm = res.queryOneResult.data;
        }
      });
    }
  }
