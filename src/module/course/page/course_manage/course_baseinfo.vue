  ...
  export default {
          ...
          this.$refs.courseForm.validate((valid) => {
            if (valid) {
              this.$confirm('确认提交吗？', '提示', {}).then(() => {
                ...
                this.courseForm.id = this.courseid;
    ...
    mounted(){
      ...
      courseApi.category_findlist().then(res=>{
        if (res.success) {
          this.categoryList = res.queryOneResult.data.children;
      ...
      systemApi.sys_getDictionary("200").then(res=>{
        ...
        this.gradeList = res.queryOneResult.data.dvalue;
      });
      //查询学习模式
      systemApi.sys_getDictionary("201").then(res=>{

        this.studymodelList = res.queryOneResult.data.dvalue;
      });
      //查询课程信息
        //课程id
...
//          console.log(res);
        this.courseForm = res.queryOneResult.data;
          //课程分类显示，需要两级分类
